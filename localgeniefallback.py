...
AI100_API_KEY = os.getenv("GENIEX_API_KEY", "geniex")  # GenieX's local server doesn't
                                                         # check this, but the header
                                                         # is still required by the
                                                         # OpenAI-compatible surface
AI100_API_URL = os.getenv("GENIEX_API_URL", "http://127.0.0.1:18181/v1/chat/completions")
AI100_MODEL = os.getenv("GROQ_MODEL", "llama-3.1-8b-instant")# ^ must match whatever you `geniex pull`ed. Confirmed available for Snapdragon X Elite
# as of this writing: ai-hub-models/Qwen3-4B-Instruct-2507 and
# ai-hub-models/llama_v3_2_3b_chat_quantized. Check `geniex pull --help` / the AI Hub
# models catalog (filter: Snapdragon X Elite + GenieX - QAIRT + Text Generation) for
# the current list, since the catalog changes.
...
def validate_cloud_output(text):
    ...
    concept = clean_concept(data.get("concept", "none"))
...
def predict_via_cloud(sentence, debug=False, max_retries=1, retry_delay=2):
    """
    max_retries: how many times to retry ONLY on a transient "model busy" error.
    retry_delay: seconds to wait between retries.

    Timeouts are intentionally short (connect=5s, read=8s). This is a real-time
    fallback for a live conversation aid -- if the cloud can't answer fast, the
    right move is to fail fast and let the caller fall back to the edge model's
    own (lower-confidence) prediction, not to block the user for a minute.
    """
    ...
    while True:
        ...
        try:
            response = requests.post(
                ...
                headers=headers,
                json=payload,
                timeout=(5, 30)  # (connect_timeout, read_timeout) -- local inference,
                                  # first call may take longer while Ollama loads the
                                  # model into memory; subsequent calls are fast since
                                  # it stays resident
            )
            ...
            if debug:
                print(f"[DEBUG] attempt={attempt} elapsed={time.time() - start:.1f}s")
                print(f"[DEBUG] status_code={response.status_code}")
                print(f"[DEBUG] body={response.text[:500]}")

            is_busy = response.status_code == 500 and "busy" in response.text.lower()
            ...
            if is_busy:
                return {
                    ...
                    "source": "cloud",
                    "reason": "model_busy",
                    "note": "Shared inference hardware is at capacity. Caller should "
                            "fall back to the edge model's own prediction rather than "
                            "keep waiting."
                }
            ...
            return {
                ...
                "source": "cloud",
                "reason": "read_timeout",
                "note": "Connected fine, but no response within 8s. On shared hackathon "
                        "hardware this usually means capacity, not your code."
            }
