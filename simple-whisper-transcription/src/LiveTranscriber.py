...
from wifi_sender import start_server, send_csv
start_server()
PROJECT_ROOT = Path(__file__).resolve().parents[2]
sys.path.insert(0, str(PROJECT_ROOT))

try:
    from haptic_inference import predict_haptic_context
except Exception as exc:
    print(f"[HAPTIC DISABLED] {exc}")
    def predict_haptic_context(transcript: str):
        return {
            "success": False,
            "reason": "haptic_inference_unavailable",
            "sentence": transcript,
            "error": str(exc),
        }
...
def process_transcription(
    ...
    try:
        ...
        # 2. TEXT -> HAPTIC ONNX MODEL
        prediction = predict_haptic_context(transcript)

        # 3. CHECK WHETHER EDGE MODEL ACCEPTED IT
        if not prediction.get("success", False):
            print(
                f"[FALLBACK REQUIRED] "
                f"Reason: {prediction.get('reason', 'unknown')}"
            )

            if prediction.get("unknown_words"):
                print(
                    f"[UNKNOWN WORDS] "
                    f"{prediction['unknown_words']}"
                )

            if prediction.get("confidence"):
                print(
                    f"[EDGE PREDICTION] "
                    f"{prediction.get('intent')} | "
                    f"{prediction.get('subject')} | "
                    f"{prediction.get('action')} | "
                    f"{prediction.get('concept')}"
                )

            # Later:
            # prediction = sarvam_fallback(transcript)

            return

        # 4. EXTRACT 4 MODEL OUTPUTS
        ...
        if prediction.get("source") == "llama_fallback":
            print("[LLM FALLBACK] low-confidence CNN output was rerouted to the local LLM")
        else:
            print("[EDGE MODEL] output came directly from the CNN")

        print(
            f"[CONFIDENCE] "
            f"Intent={prediction['confidence']['intent']:.3f} | "
            f"Subject={prediction['confidence']['subject']:.3f} | "
            f"Action={prediction['confidence']['action']:.3f} | "
            f"Concept={prediction['confidence']['concept']:.3f}"
        )

        # 5. SEND OUTPUT TO HAPTIC HARDWARE
        # This is the next stage we will implement:
        #
        # send_haptic_output(
        #     intent,
        #     subject,
...
class LiveTranscriber:
    def __init__(self):
        ...
        # model paths
        self.encoder_path = PROJECT_ROOT / config.get(
            "encoder_path",
            "models/WhisperEncoder.onnx"
        )

        self.decoder_path = PROJECT_ROOT / config.get(
            "decoder_path",
            "models/WhisperDecoder.onnx"
        )

        self.encoder_path = str(self.encoder_path)
        self.decoder_path = str(self.decoder_path)
